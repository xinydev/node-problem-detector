...
IMAGE:=$(REGISTRY)/node-problem-detector:v1
...
build-container: build-binaries Dockerfile
	#docker buildx build --platform linux/amd64,linux/arm/v7 --push -t xinydev/node-problem-detector:v1 --build-arg BASEIMAGE=$(BASEIMAGE) --build-arg LOGCOUNTER=$(LOGCOUNTER) .
	docker buildx build --platform linux/amd64,linux/arm/v7 --load -t xinydev/node-problem-detector:v1 --build-arg BASEIMAGE=$(BASEIMAGE) --build-arg LOGCOUNTER=$(LOGCOUNTER) .
